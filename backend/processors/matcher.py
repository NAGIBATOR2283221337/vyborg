...
def pick_showtimes_for_report_title(title: str, index: Dict[Tuple[str, frozenset], List[datetime]]) -> List[datetime]:
    ...
    1. Точное совпадение по базе и полному набору эпизодов
    2. Частичное пересечение эпизодов (если включено ALLOW_EPISODE_PARTIAL)
    3. Совпадение по базе без эпизодов
    ...
    # Стратегия 1: Точное совпадение базы и всех эпизодов
    if eps_r_set:
        for b, e in cands:
            if e == frozenset(eps_r_set):
                logger.debug(f"✅ Точное совпадение эпизодов: '{title}' → '{b}' eps={e}")
                return index[(b, e)]

    # Стратегия 2: Частичное пересечение эпизодов (любая общая серия подходит)
    if ALLOW_EPISODE_PARTIAL and eps_r_set:
        ...
        if out:
            logger.debug(f"✅ Частичное совпадение эпизодов: '{title}' → {matched_keys}")
            return sorted(set(out))

    # Стратегия 3: Совпадение по базе без учета эпизодов (для передач без серий)
    if not eps_r_set:
        for b, e in cands:
            if not e or e == frozenset(['__NOSER__']):
                logger.debug(f"✅ Совпадение без эпизодов: '{title}' → '{b}'")
                return index[(b, e)]

    # Стратегия 4: Берем топ-кандидата с любыми эпизодами (fallback)
    # Это позволяет найти хотя бы что-то, даже если эпизоды не совпадают
    if cands:
        b, e = cands[0]
        logger.debug(f"⚠️ Fallback: '{title}' → '{b}' eps={e}")
        return index[(b, e)]
